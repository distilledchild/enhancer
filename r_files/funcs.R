  ...
  header.line <- tryCatch(
    ...
    error = function(e) {
      stop(
        ...
        "If it is stored in Dropbox, select Make available offline.\n",
...
normalize_hiccups_loop_coordinates <- function(
  ...
  df.sample.loop.coordinate.normalized <- df.sample.loop.raw %>%
    transmute(
      sample,
      strain,
      source_file,
  ...
  validate_one_based_inclusive_intervals(
    df.sample.loop.coordinate.normalized$start1,
    df.sample.loop.coordinate.normalized$end1,
    "Normalized HiCCUPS anchor1 intervals"
  )
  validate_one_based_inclusive_intervals(
    df.sample.loop.coordinate.normalized$start2,
    df.sample.loop.coordinate.normalized$end2,
    "Normalized HiCCUPS anchor2 intervals"
  )

  expected.anchor.width <- case_when(
    df.sample.loop.coordinate.normalized$resolution == "5K" ~ 5000L,
    df.sample.loop.coordinate.normalized$resolution == "10K" ~ 10000L,
    df.sample.loop.coordinate.normalized$resolution == "25K" ~ 25000L,
    TRUE ~ NA_integer_
  )
  observed.anchor1.width <- df.sample.loop.coordinate.normalized$end1 -
    df.sample.loop.coordinate.normalized$start1 + 1L
  observed.anchor2.width <- df.sample.loop.coordinate.normalized$end2 -
    df.sample.loop.coordinate.normalized$start2 + 1L
...
build_pooled_hiccups_loop_resource <- function(
  ...
  df.loop.universe <- df.loop.distinct.coordinate.normalized %>%
    filter(passes_lt2mb)
  ...
  list(
    ...
    distinct = df.loop.distinct.coordinate.normalized,
