...
resolve_enhancer_r_files_dir <- function() {
  ...
  script.candidate <- if (is.na(script.path)) NA_character_ else dirname(script.path)
  candidates <- unique(c(
...
output.dir <- path.expand(Sys.getenv(
  ...
  unset = file.path(default.output.parent, "downsampling_140M_outputs")
))
...
depth.series.output.dir <- path.expand(Sys.getenv(
  ...
  unset = file.path(default.output.parent, "downsampling_140M_210M_outputs")
))
...
downsample.210m.root <- resolve_optional_downsample_root(c(
  ...
  path.expand(
    paste0(
      "~/Library/CloudStorage/GoogleDrive-wellclouder@gmail.com/",
      "My Drive/juicer_downsample_q30_210M"
    )
  ),
  path.expand(
