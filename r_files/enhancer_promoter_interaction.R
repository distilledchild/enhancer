...
df.final.up.down.directional.point.decision <- df.final.up.down.directional.point %>%
  separate(
    col = loop.id,
...
df.final.up.down.directional.point.decision %>% 
  summarise(n = mean(abs(gene_start - gene_end), na.rm = TRUE))

###################################
# 1. ONE OK case in either anchor
###################################

df.final.up.down.directional.point.decision %>% distinct(loop.id) %>% count()
df.final.up.down.directional.point.decision %>% 
  filter(classification == "One_OK") %>% # 
  # distinct(gene_id) %>% # 7,894
  distinct(loop.id) %>% # 11,937/31,019
  count()
...
# 1 7400.  23371 66692  6459.  19056 49410.
...
# Using Q3 as threshold for distance filtering
...
df.final.up.down.directional.point.decision.both.OK <- df.final.up.down.directional.point.decision %>% 
  filter(classification == "Both_OK") #%>% # 3,801/31,019
  # distinct(loop.id) %>% 
  # count()

df.final.up.down.directional.point.decision.both.OK %>% head(2)
df.final.up.down.directional.point.decision.both.OK %>% dim() # 7,602 // 3,801 * 2
...
ggplot(dist.diff.decision.both.OK, aes(x = "", y = dist_diff)) +
  ...
  annotate("text", x = 0.8, y = stats_dist_diff$Q1, label = paste0("Q1=", stats_dist_diff$Q1),
           hjust = 1, vjust = -0.5, color = "red") +
  annotate("text", x = 0.8, y = stats_dist_diff$Q3, label = paste0("Q3=", stats_dist_diff$Q3),
...
df.final.up.down.directional.point.decision.both.OK.filtered %>% dim() # 3,810
...
df.final.up.down.directional.point.decision.both.OK.tied
...
ggplot(df.final.up.down.directional.point.decision.both.OK, aes(x = distance)) +
  ...
  annotate("text", x = stats_decision$Q3, y = 0, label = paste0("Q3=", round(stats_decision$Q3,0)),
           angle = 90, vjust = -0.5, hjust = 0, color = "red")

df.final.up.down.directional.point.decision.both.OK.filtered %>%
  summarise(
    perc_below_Q3 = mean(distance < 66692) * 100
  )
...
df.final.up.down.directional.point.decision # 61,974
...
df.final.up.down.directional.point.decision.COMBINED.OK.filtered <- bind_rows(df.final.up.down.directional.point.decision.one.OK.filtered, 
  df.final.up.down.directional.point.decision.both.OK.filtered.lt.Q3)

df.final.up.down.directional.point.decision.COMBINED.OK.filtered %>% dim() # 15,667 (11,937 + 3,730 = 15667: PASS!)
