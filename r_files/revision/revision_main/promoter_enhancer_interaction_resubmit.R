...
if (!file.exists(coord.prep.script)) {
  stop(
    "Cannot locate the coordinate-preparation script: ",
    coord.prep.script,
    call. = FALSE
  )
}
...
force.coord.cache.rebuild <- tolower(
  Sys.getenv("REBUILD_COORD_CACHE", unset = "0")
) %in% c("1", "true", "yes")

# Rebuild the cache only when forced or when expected objects are missing.
if (
  force.coord.cache.rebuild ||
    !coordinate_cache_complete(
      cache.dir = coord.cache.dir,
      object.names = coord.cache.object.names
    )
) {
  if (force.coord.cache.rebuild) {
    message("Rebuilding coordinate cache because REBUILD_COORD_CACHE is set.")
  } else {
    message("Coordinate cache is incomplete; running coordinate preparation.")
  }

  source(coord.prep.script, local = environment())
}

# Confirm that coordinate preparation produced every expected cache object.
if (
  !coordinate_cache_complete(
    cache.dir = coord.cache.dir,
    object.names = coord.cache.object.names
  ...
  stop(
    "Coordinate preparation finished without producing a complete cache.",
    call. = FALSE
  )
}

load_coordinate_cache_objects(
  cache.dir = coord.cache.dir,
  object.names = coord.cache.object.names,
  envir = environment()
)
...
required.cached.input.names <- c(
  "library_complexity",
  "genetic_distance"
)
missing.cached.input.names <- setdiff(
  required.cached.input.names,
  names(input.file.paths)
)
# Require all downstream file-path entries recorded during coordinate prep.
if (length(missing.cached.input.names) > 0L) {
  stop(
    "Cached input table is missing required path name(s): ",
    paste(missing.cached.input.names, collapse = ", "),
    call. = FALSE
  )
}
...
genetic.distance.file <- Sys.getenv(
  "HRDP_GENETIC_DISTANCE_FILE",
  unset = input.file.paths[["genetic_distance"]]
)

required.analysis.files <- library.complexity.file
missing.analysis.files <- required.analysis.files[
  !file.exists(required.analysis.files)
]
# Stop before analysis when a required downstream input is unavailable.
if (length(missing.analysis.files) > 0L) {
  stop(
    "Missing required downstream analysis file(s):\n",
...
message(
  "Loaded ",
  length(coord.cache.object.names),
...
#
# Ensembl GTF transcript records are already normalized to rn7, chr-prefixed,
# one-based inclusive coordinates in the Step 1 cache. A true transcript TSS is
# the transcript_start on the plus strand and transcript_end on the minus strand.
# GTF genomic intervals always satisfy transcript_start <= transcript_end on
# both strands. Minus-strand transcription begins at the numerically larger
# transcript_end coordinate and proceeds toward decreasing genomic coordinates.
# Every transcript-level TSS is retained for the upcoming direct-overlap step.
################################################################################
...
df.true.tss.summary <- tibble(
  metric = c(
    "normalized_Ensembl_transcripts",
    "transcript_level_true_TSS_records",
    "unique_Ensembl_genes",
    "unique_strand_aware_TSS_sites",
    "unique_genomic_TSS_positions",
    "Ensembl_canonical_transcript_TSS_records",
    "genes_with_Ensembl_canonical_transcript_TSS",
    "plus_strand_transcript_TSS",
    "minus_strand_transcript_TSS",
    "transcripts_with_start_le_end",
    "plus_strand_TSS_at_transcript_start",
    "minus_strand_TSS_at_transcript_end",
  ...
  n = c(
    ...
    n_distinct(
      df.true.tss.transcript$chr,
      df.true.tss.transcript$true_tss_start,
      df.true.tss.transcript$strand
    ),
    n_distinct(
      df.true.tss.transcript$chr,
      df.true.tss.transcript$true_tss_start
    ),
    sum(df.true.tss.transcript$is_ensembl_canonical),
    n_distinct(
      df.true.tss.transcript$gene_id[
        df.true.tss.transcript$is_ensembl_canonical
      ]
    ),
    sum(df.true.tss.transcript$strand == "+"),
    sum(df.true.tss.transcript$strand == "-"),
    sum(
      df.true.tss.transcript$transcript_start <=
...
message(
  "Generated ",
  nrow(df.true.tss.transcript),
  " strand-aware transcript TSS records across ",
  n_distinct(df.true.tss.transcript$gene_id),
  " Ensembl genes."
)
...
#
# Strict sensitivity evidence uses one-base Ensembl TSS points and original
# 81-bp EPD promoter intervals. Primary evidence requires anchor overlap with a
# TSS +/-1-kb promoter window. Both anchors and every overlapping gene/transcript
# are retained without nearest-feature selection, LOC removal, or manual ties.
# Loops annotated at both anchors remain explicit; direction is not forced.
################################################################################

# Construct both anchor sets once and retain compatibility aliases for analyses
# that require one specific side.
gr.loop.anchor.by.side <- create_loop_anchor_granges_by_side(
  df.loop.universe
)
...
df.strict.direct.promoter.tss.anchor.count <-
  strict.direct.summary$anchor_count
df.strict.direct.promoter.tss.anchor.summary <-
  strict.direct.summary$anchor_summary
df.strict.direct.promoter.tss.anchor.wide <-
  strict.direct.summary$anchor_wide
df.strict.direct.promoter.tss.loop.gene.count <-
  strict.direct.summary$loop_gene_count
df.strict.direct.promoter.tss.loop.summary <-
  strict.direct.summary$loop_summary
...
gr.epd.tss.promoter.window.1kb <- expand_tss_to_promoter_windows(
  gr.epd.tss,
  flank.bp = promoter.window.flank.bp
)
...
df.direct.promoter.tss.gene.assignment <-
  primary.direct.summary$gene_assignment
df.loop.anchor.index <- primary.direct.summary$anchor_index
df.direct.promoter.tss.anchor.count <-
  primary.direct.summary$anchor_count
df.direct.promoter.tss.anchor.summary <-
  primary.direct.summary$anchor_summary
df.direct.promoter.tss.anchor.wide <-
  primary.direct.summary$anchor_wide
df.direct.promoter.tss.loop.gene.count <-
  primary.direct.summary$loop_gene_count
df.direct.promoter.tss.loop.summary <-
  primary.direct.summary$loop_summary
df.direct.promoter.tss.summary <-
  primary.direct.summary$summary

# Compare strict and primary definitions without treating either as functional
# validation. The comparison records both loop-level and assignment-level gains.
df.strict.vs.primary.promoter.window.loop.comparison <-
  ...
  left_join(
    df.strict.direct.promoter.tss.loop.summary %>%
      transmute(
        loop_id,
        strict_exact_direct = has_any_direct_promoter_tss
      ),
    by = "loop_id"
  ) %>%
  left_join(
    df.direct.promoter.tss.loop.summary %>%
      transmute(
        loop_id,
        primary_1kb_direct = has_any_direct_promoter_tss
      ),
    by = "loop_id"
  ) %>%
  mutate(
...
df.strict.vs.primary.promoter.window.summary <- bind_rows(
  df.strict.direct.promoter.tss.summary %>%
    mutate(evidence_definition = "strict_exact_TSS_or_81bp_EPD", .before = 1),
  df.direct.promoter.tss.summary %>%
    mutate(evidence_definition = "primary_TSS_plus_minus_1kb", .before = 1)
) %>%
  bind_rows(
    tibble(
...
#
# Proximal evidence is deliberately weaker than primary direct overlap. The
# complete exploratory catalog includes all non-overlapping Ensembl TSS points
# and EPD promoter intervals within 200 kb of an anchor. A secondary tier is
# then restricted to inward candidates within 10 kb that lie on the assigned
# anchor's side of the loop midpoint and are strictly closer to that anchor than
# to the opposite anchor. No one-gene-per-anchor selection is imposed.
################################################################################
...
df.proximal.true.tss.anchor.pair <-
  df.proximal.true.tss.anchor.hit %>%
  ...
  transmute(
    ...
    proximal_distance_tier = classify_proximal_distance_tier(
      anchor_annotation_edge_distance_bp
    ),
    gene_id,
    gene_id_versioned,
...
df.proximal.epd.promoter.anchor.pair <-
  df.proximal.epd.promoter.anchor.hit %>%
...
df.proximal.promoter.tss.anchor.pair <- bind_rows(
  ...
  left_join(
    df.loop.universe %>%
      dplyr::select(
        loop_id,
        loop_chr1 = chr1,
        loop_anchor1_start = start1,
        loop_anchor1_end = end1,
        loop_chr2 = chr2,
        loop_anchor2_start = start2,
        loop_anchor2_end = end2
      ),
    by = "loop_id"
  ) %>%
  mutate(
    ...
    loop_partition_midpoint = (
      loop_anchor1_end + loop_anchor2_start
    ) / 2,
    annotation_midpoint = (
      annotation_start + annotation_end
    ) / 2,
    has_nonempty_inter_anchor_interval = (
      loop_chr1 == loop_chr2 & inter_anchor_start <= inter_anchor_end
    ),
    annotation_fully_within_inter_anchor = (
      has_nonempty_inter_anchor_interval &
        annotation_chr == loop_chr1 &
        annotation_start >= inter_anchor_start &
        annotation_end <= inter_anchor_end
    ),
    annotation_points_toward_loop_interior = case_when(
      anchor_side == "anchor1" ~ annotation_relative_to_anchor ==
        "right_of_anchor",
      anchor_side == "anchor2" ~ annotation_relative_to_anchor ==
        "left_of_anchor",
      TRUE ~ FALSE
    ),
    distance_to_anchor1_edge_bp = if_else(
      annotation_fully_within_inter_anchor,
      annotation_start - loop_anchor1_end,
      NA_integer_
    ),
    distance_to_anchor2_edge_bp = if_else(
      annotation_fully_within_inter_anchor,
      loop_anchor2_start - annotation_end,
      NA_integer_
    ),
    ...
    annotation_fully_on_assigned_midpoint_side = case_when(
      anchor_side == "anchor1" ~
        annotation_end < loop_partition_midpoint,
      anchor_side == "anchor2" ~
        annotation_start > loop_partition_midpoint,
      TRUE ~ FALSE
    ),
    annotation_spans_loop_midpoint = (
      annotation_start <= loop_partition_midpoint &
        annotation_end >= loop_partition_midpoint
    ),
    assigned_anchor_is_strictly_nearest = (
      !is.na(assigned_anchor_edge_distance_bp) &
        !is.na(opposite_anchor_edge_distance_bp) &
        assigned_anchor_edge_distance_bp <
          opposite_anchor_edge_distance_bp
    ),
    anchor_edge_distance_tie = (
      !is.na(assigned_anchor_edge_distance_bp) &
        !is.na(opposite_anchor_edge_distance_bp) &
        assigned_anchor_edge_distance_bp ==
          opposite_anchor_edge_distance_bp
    ),
    is_inward_proximal_evidence = (
      annotation_fully_within_inter_anchor &
        annotation_points_toward_loop_interior
    ),
    is_outward_proximal_evidence = !is_inward_proximal_evidence,
    is_inward_proximal_evidence_10kb = (
      is_inward_proximal_evidence &
        anchor_annotation_edge_distance_bp <= 10000L
    ),
    is_midpoint_nearest_inward_proximal_evidence = (
      is_inward_proximal_evidence &
        annotation_fully_on_assigned_midpoint_side &
        assigned_anchor_is_strictly_nearest
    ),
    is_midpoint_nearest_inward_proximal_evidence_10kb = (
      is_midpoint_nearest_inward_proximal_evidence &
        anchor_annotation_edge_distance_bp <= 10000L
    ),
    is_ambiguous_inward_proximal_evidence = (
      is_inward_proximal_evidence &
        (
          annotation_spans_loop_midpoint |
            anchor_edge_distance_tie
  ...
  mutate(
    distance_rank_within_anchor_annotation_class = dense_rank(
      anchor_annotation_edge_distance_bp
    ),
    is_nearest_within_anchor_annotation_class =
      distance_rank_within_anchor_annotation_class == 1L
  ) %>%
  ungroup() %>%
  arrange(
    loop_id,
...
assert_analysis_unique_key(
  ...
  "proximal_evidence_id",
  "Proximal promoter/TSS evidence identifiers are not unique."
)
...
assert_analysis_condition(
  !any(is.na(df.proximal.promoter.tss.anchor.pair$proximal_distance_tier)) &&
    ...
    !any(
      df.proximal.promoter.tss.anchor.pair$
        anchor_annotation_edge_distance_bp > proximal.max.distance.bp
    ) &&
    !any(
      df.proximal.promoter.tss.anchor.pair$
        is_inward_proximal_evidence &
...
df.proximal.promoter.tss.gene.assignment <-
  df.proximal.promoter.tss.anchor.pair %>%
  group_by(
    loop_id,
    resolution,
  ...
  summarise(
    ...
    min_proximal_distance_bp = min(
      anchor_annotation_edge_distance_bp
    ),
    n_proximal_evidence_records = n(),
    has_proximal_true_tss = any(annotation_class == "true_TSS"),
    has_proximal_epd_promoter = any(
      annotation_class == "EPD_promoter"
    ),
    n_proximal_true_tss_transcripts = n_distinct(
      transcript_id_versioned,
      na.rm = TRUE
    ),
    n_proximal_epd_promoters = n_distinct(
      promoter_annotation_id,
      na.rm = TRUE
    ),
    ...
    has_inward_proximal_evidence_10kb = any(
      is_inward_proximal_evidence_10kb
    ),
    has_midpoint_nearest_inward_proximal_evidence = any(
      is_midpoint_nearest_inward_proximal_evidence
    ),
    has_midpoint_nearest_inward_proximal_evidence_10kb = any(
      is_midpoint_nearest_inward_proximal_evidence_10kb
    ),
    has_ambiguous_inward_proximal_evidence = any(
      is_ambiguous_inward_proximal_evidence
    ),
    min_inward_proximal_distance_bp = if (
      any(is_inward_proximal_evidence)
    ) {
      min(
        anchor_annotation_edge_distance_bp[
          is_inward_proximal_evidence
    ...
    min_midpoint_nearest_inward_proximal_distance_bp = if (
      any(is_midpoint_nearest_inward_proximal_evidence)
    ) {
      min(
        anchor_annotation_edge_distance_bp[
          is_midpoint_nearest_inward_proximal_evidence
  ...
  left_join(
    df.direct.promoter.tss.gene.assignment %>%
      transmute(
        ...
        anchor_side,
        gene_id,
        has_direct_same_anchor_gene = TRUE
      ),
    by = c("loop_id", "anchor_side", "gene_id")
  ) %>%
  mutate(
    has_direct_same_anchor_gene = coalesce(
      has_direct_same_anchor_gene,
      FALSE
    ),
    is_secondary_inward_proximal_candidate_10kb = (
      has_midpoint_nearest_inward_proximal_evidence_10kb &
        !has_direct_same_anchor_gene
    )
  ) %>%
  group_by(loop_id, anchor_side) %>%
  mutate(
    proximal_gene_distance_rank_at_anchor = dense_rank(
      min_proximal_distance_bp
    ),
    is_nearest_proximal_gene_at_anchor =
      proximal_gene_distance_rank_at_anchor == 1L
  ) %>%
  ungroup() %>%
  arrange(
    loop_id,
...
df.secondary.inward.proximal.gene.assignment <-
  ...
  arrange(
    loop_id,
    anchor_side,
    min_midpoint_nearest_inward_proximal_distance_bp,
    gene_id
  )

df.proximal.promoter.tss.anchor.count <-
  df.proximal.promoter.tss.anchor.pair %>%
  ...
  summarise(
    ...
    n_proximal_epd_promoter_records = sum(
      annotation_class == "EPD_promoter"
    ),
    n_proximal_1bp_to_10kb = sum(
      proximal_distance_tier == "01_1bp_to_10kb"
    ),
    n_proximal_gt10kb_to_50kb = sum(
      proximal_distance_tier == "02_gt10kb_to_50kb"
    ),
    n_proximal_gt50kb_to_200kb = sum(
      proximal_distance_tier == "03_gt50kb_to_200kb"
    ),
    n_inward_proximal_evidence_records = sum(
      is_inward_proximal_evidence
    ),
    n_outward_proximal_evidence_records = sum(
      is_outward_proximal_evidence
    ),
    n_inward_proximal_evidence_records_10kb = sum(
      is_inward_proximal_evidence_10kb
    ),
    n_midpoint_nearest_inward_proximal_evidence_records = sum(
      is_midpoint_nearest_inward_proximal_evidence
    ),
    n_midpoint_nearest_inward_proximal_evidence_records_10kb = sum(
      is_midpoint_nearest_inward_proximal_evidence_10kb
    ),
    n_ambiguous_inward_proximal_evidence_records = sum(
      is_ambiguous_inward_proximal_evidence
    ),
    .groups = "drop"
  )

df.secondary.inward.proximal.anchor.count <-
  df.secondary.inward.proximal.gene.assignment %>%
  group_by(loop_id, resolution, anchor_side, opposite_anchor_side) %>%
  summarise(
    n_secondary_inward_proximal_genes_10kb = n_distinct(gene_id),
    min_secondary_inward_proximal_distance_bp = min(
      min_midpoint_nearest_inward_proximal_distance_bp
    ),
    .groups = "drop"
  )
...
df.proximal.promoter.tss.anchor.summary <- df.loop.anchor.index %>%
  ...
  mutate(
    across(
      c(
        n_proximal_evidence_records,
        n_proximal_genes,
        n_proximal_true_tss_records,
        n_proximal_epd_promoter_records,
        n_proximal_1bp_to_10kb,
        n_proximal_gt10kb_to_50kb,
        n_proximal_gt50kb_to_200kb,
        n_inward_proximal_evidence_records,
        n_outward_proximal_evidence_records,
        n_inward_proximal_evidence_records_10kb,
        n_midpoint_nearest_inward_proximal_evidence_records,
        n_midpoint_nearest_inward_proximal_evidence_records_10kb,
        n_ambiguous_inward_proximal_evidence_records,
        n_secondary_inward_proximal_genes_10kb
      ),
      ~ coalesce(.x, 0L)
    ),
    has_any_proximal_promoter_tss = n_proximal_evidence_records > 0L,
    has_any_inward_proximal_promoter_tss =
      n_inward_proximal_evidence_records > 0L,
    has_secondary_inward_proximal_10kb =
      n_secondary_inward_proximal_genes_10kb > 0L,
    min_proximal_distance_bp = if_else(
      has_any_proximal_promoter_tss,
      min_proximal_distance_bp,
      NA_integer_
    ),
    min_secondary_inward_proximal_distance_bp = if_else(
      has_secondary_inward_proximal_10kb,
      min_secondary_inward_proximal_distance_bp,
      NA_integer_
    )
  ) %>%
...
df.proximal.promoter.tss.anchor.wide <-
  df.proximal.promoter.tss.anchor.summary %>%
  dplyr::select(
    loop_id,
    anchor_side,
...
df.proximal.promoter.tss.loop.gene.count <-
  df.proximal.promoter.tss.gene.assignment %>%
  group_by(loop_id) %>%
  summarise(
...
df.secondary.inward.proximal.loop.gene.count <-
  df.secondary.inward.proximal.gene.assignment %>%
  group_by(loop_id) %>%
  summarise(
    n_secondary_inward_proximal_genes_10kb_across_anchors =
      n_distinct(gene_id),
    .groups = "drop"
  )
...
df.proximal.promoter.tss.loop.summary <- df.loop.universe %>%
  ...
  mutate(
    n_proximal_genes_across_anchors = coalesce(
      n_proximal_genes_across_anchors,
      0L
    ),
    n_secondary_inward_proximal_genes_10kb_across_anchors = coalesce(
      n_secondary_inward_proximal_genes_10kb_across_anchors,
      0L
    ),
    n_proximal_anchor_sides =
      as.integer(has_any_proximal_promoter_tss_anchor1) +
        as.integer(has_any_proximal_promoter_tss_anchor2),
    has_any_proximal_promoter_tss = n_proximal_anchor_sides > 0L,
    has_proximal_promoter_tss_both_anchors =
      n_proximal_anchor_sides == 2L,
    has_any_direct_or_proximal_promoter_tss = (
      has_any_direct_promoter_tss | has_any_proximal_promoter_tss
    ),
    n_secondary_inward_proximal_anchor_sides_10kb =
      as.integer(has_secondary_inward_proximal_10kb_anchor1) +
        as.integer(has_secondary_inward_proximal_10kb_anchor2),
    has_any_secondary_inward_proximal_10kb =
      n_secondary_inward_proximal_anchor_sides_10kb > 0L,
    has_secondary_inward_proximal_both_anchors_10kb =
      n_secondary_inward_proximal_anchor_sides_10kb == 2L,
    promoter_tss_assignment_tier = case_when(
      has_any_direct_promoter_tss &
        has_any_secondary_inward_proximal_10kb ~
        "primary_direct_and_secondary_inward_10kb",
      has_any_direct_promoter_tss ~ "direct_only",
      has_any_secondary_inward_proximal_10kb ~
        "secondary_inward_10kb_only",
...
df.primary.direct.promoter.site.count.by.anchor <-
  df.direct.promoter.tss.anchor.overlap %>%
  mutate(
    assigned_tss_coordinate = coalesce(tss_start, epd_tss_start),
    promoter_site_key = str_c(
      gene_id,
      annotation_chr,
      assigned_tss_coordinate,
      sep = "|"
    )
  ) %>%
  group_by(loop_id, anchor_side) %>%
...
df.secondary.promoter.site.count.by.anchor <-
  df.proximal.promoter.tss.anchor.pair %>%
...
df.exploratory.promoter.site.count.by.anchor <-
  df.proximal.promoter.tss.anchor.pair %>%
  mutate(
    assigned_tss_coordinate = coalesce(epd_tss_start, annotation_start),
    promoter_site_key = str_c(
...
df.promoter.tss.exclusive.loop.category <- df.loop.universe %>%
  left_join(
    df.strict.direct.promoter.tss.loop.summary %>%
      transmute(
        loop_id,
        n_strict_exact_anchor_sides = n_direct_anchor_sides,
        has_strict_exact_direct_overlap = has_any_direct_promoter_tss
      ),
    by = "loop_id"
  ) %>%
  left_join(
    df.direct.promoter.tss.loop.summary %>%
      transmute(
        loop_id,
        n_primary_1kb_anchor_sides = n_direct_anchor_sides,
        has_primary_1kb_direct_overlap = has_any_direct_promoter_tss
      ),
    by = "loop_id"
  ) %>%
  left_join(
  ...
  mutate(
    across(
      matches("^n_.*promoter_sites_anchor[12]$"),
      ~ coalesce(.x, 0L)
    ),
    mutually_exclusive_category = case_when(
      has_strict_exact_direct_overlap &
        n_primary_1kb_anchor_sides == 1L ~
        "01_strict_exact_direct_primary_single_anchor",
      has_strict_exact_direct_overlap &
        n_primary_1kb_anchor_sides == 2L ~
        "02_strict_exact_direct_primary_both_anchors",
      !has_strict_exact_direct_overlap &
        n_primary_1kb_anchor_sides == 1L ~
        "03_primary_1kb_window_only_single_anchor",
      !has_strict_exact_direct_overlap &
        n_primary_1kb_anchor_sides == 2L ~
        "04_primary_1kb_window_only_both_anchors",
      !has_primary_1kb_direct_overlap &
        n_secondary_inward_proximal_anchor_sides_10kb == 1L ~
        "05_secondary_midpoint_nearest_10kb_single_anchor",
      !has_primary_1kb_direct_overlap &
        n_secondary_inward_proximal_anchor_sides_10kb == 2L ~
        "06_secondary_midpoint_nearest_10kb_both_anchors",
      !has_primary_1kb_direct_overlap &
        !has_any_secondary_inward_proximal_10kb &
        has_any_proximal_promoter_tss ~
        "07_exploratory_other_proximity_1bp_to_200kb",
      TRUE ~ "08_no_promoter_TSS_anchor_or_proximity_support"
    ),
    category_description = case_when(
      mutually_exclusive_category ==
        "01_strict_exact_direct_primary_single_anchor" ~ paste0(
        "Strict exact TSS/EPD overlap is present and the primary +/-1-kb ",
        "definition supports exactly one anchor."
      ),
      mutually_exclusive_category ==
        "02_strict_exact_direct_primary_both_anchors" ~ paste0(
        "Strict exact TSS/EPD overlap is present and the primary +/-1-kb ",
        "definition supports both anchors; promoter-promoter compatible."
      ),
      mutually_exclusive_category ==
        "03_primary_1kb_window_only_single_anchor" ~ paste0(
        "No strict exact overlap; a TSS +/-1-kb promoter window supports ",
        "exactly one anchor."
      ),
      mutually_exclusive_category ==
        "04_primary_1kb_window_only_both_anchors" ~ paste0(
        "No strict exact overlap; TSS +/-1-kb promoter windows support both ",
        "anchors; promoter-promoter compatible."
      ),
      mutually_exclusive_category ==
        "05_secondary_midpoint_nearest_10kb_single_anchor" ~ paste0(
        "No primary direct overlap; a non-overlapping TSS/EPD promoter is ",
        "within 10 kb, on the assigned midpoint side, and strictly closer ",
        "to one anchor."
      ),
      mutually_exclusive_category ==
        "06_secondary_midpoint_nearest_10kb_both_anchors" ~ paste0(
        "No primary direct overlap; distinct midpoint-resolved nearest ",
        "TSS/EPD candidates support both anchors within 10 kb."
      ),
      mutually_exclusive_category ==
        "07_exploratory_other_proximity_1bp_to_200kb" ~ paste0(
        "No primary or resolved secondary support; other non-overlapping ",
        "TSS/EPD proximity is present within 200 kb."
      ),
      TRUE ~ paste0(
        "No direct promoter/TSS anchor overlap and no non-overlapping ",
        "TSS/EPD promoter within the exploratory 200-kb search range."
      )
    ),
    n_category_promoter_sites_anchor1 = case_when(
      str_detect(mutually_exclusive_category, "^(01|02|03|04)_") ~
        n_primary_direct_promoter_sites_anchor1,
      str_detect(mutually_exclusive_category, "^(05|06)_") ~
        n_secondary_promoter_sites_anchor1,
      mutually_exclusive_category ==
        "07_exploratory_other_proximity_1bp_to_200kb" ~
        n_exploratory_promoter_sites_anchor1,
      TRUE ~ 0L
    ),
    n_category_promoter_sites_anchor2 = case_when(
      str_detect(mutually_exclusive_category, "^(01|02|03|04)_") ~
        n_primary_direct_promoter_sites_anchor2,
      str_detect(mutually_exclusive_category, "^(05|06)_") ~
        n_secondary_promoter_sites_anchor2,
      mutually_exclusive_category ==
        "07_exploratory_other_proximity_1bp_to_200kb" ~
        n_exploratory_promoter_sites_anchor2,
      TRUE ~ 0L
    ),
    anchor_promoter_site_multiplicity = case_when(
      pmax(
        n_category_promoter_sites_anchor1,
        n_category_promoter_sites_anchor2
      ) == 0L ~ "no_assigned_promoter_site",
      pmax(
        n_category_promoter_sites_anchor1,
        n_category_promoter_sites_anchor2
      ) == 1L ~ "one_promoter_site_at_each_supported_anchor",
      TRUE ~ "multiple_promoter_sites_at_one_or_more_anchors"
    ),
    final_P_E_input_OK = if_else(
      mutually_exclusive_category %in% c(
        "01_strict_exact_direct_primary_single_anchor",
        "03_primary_1kb_window_only_single_anchor"
      ),
      "OK",
...
df.promoter.tss.exclusive.loop.category.summary <-
  df.promoter.tss.exclusive.loop.category %>%
  group_by(
    mutually_exclusive_category,
    category_description,
    anchor_promoter_site_multiplicity,
    final_P_E_input_OK
  ) %>%
  summarise(
    n_loops = n(),
...
assert_analysis_condition(
  nrow(df.promoter.tss.exclusive.loop.category) ==
    nrow(df.loop.universe) &&
    sum(df.promoter.tss.exclusive.loop.category.summary$n_loops) ==
      nrow(df.loop.universe) &&
    !any(
      df.promoter.tss.exclusive.loop.category$
        has_strict_exact_direct_overlap &
...
df.dual.primary.anchor.representative.score <-
  ...
  mutate(
    ...
    tss_to_anchor_midpoint_distance_bp = abs(
      assigned_tss_coordinate - anchor_midpoint
    ),
    promoter_site_key = str_c(
      gene_id,
      annotation_chr,
  ...
  summarise(
    ...
    min_tss_to_anchor_midpoint_distance_bp = min(
      tss_to_anchor_midpoint_distance_bp
    ),
    closest_promoter_site_keys = str_c(
      sort(
        unique(
          promoter_site_key[
            tss_to_anchor_midpoint_distance_bp ==
              min(tss_to_anchor_midpoint_distance_bp)
          ]
        )
      ),
      collapse = ";"
  ...
  left_join(
    df.strict.direct.promoter.tss.anchor.summary %>%
      transmute(
        loop_id,
        anchor_side,
        has_strict_exact_anchor_support = has_any_direct_promoter_tss
      ),
  ...
  pivot_wider(
    names_from = anchor_side,
    values_from = c(
      has_strict_exact_anchor_support,
      n_unique_promoter_sites,
      n_unique_genes,
      min_tss_to_anchor_midpoint_distance_bp,
      closest_promoter_site_keys
    ),
    names_glue = "{.value}_{anchor_side}"
  ) %>%
  left_join(
    df.promoter.tss.exclusive.loop.category %>%
      dplyr::select(loop_id, mutually_exclusive_category),
    by = "loop_id"
  ) %>%
  mutate(
    representative_promoter_anchor = case_when(
      has_strict_exact_anchor_support_anchor1 &
        !has_strict_exact_anchor_support_anchor2 ~ "anchor1",
      !has_strict_exact_anchor_support_anchor1 &
        has_strict_exact_anchor_support_anchor2 ~ "anchor2",
      min_tss_to_anchor_midpoint_distance_bp_anchor1 <
        min_tss_to_anchor_midpoint_distance_bp_anchor2 ~ "anchor1",
      min_tss_to_anchor_midpoint_distance_bp_anchor2 <
        min_tss_to_anchor_midpoint_distance_bp_anchor1 ~ "anchor2",
      TRUE ~ NA_character_
    ),
    representative_anchor_selection_reason = case_when(
      has_strict_exact_anchor_support_anchor1 &
        !has_strict_exact_anchor_support_anchor2 ~
        "anchor1_has_strict_exact_support_only",
      !has_strict_exact_anchor_support_anchor1 &
        has_strict_exact_anchor_support_anchor2 ~
        "anchor2_has_strict_exact_support_only",
      !is.na(representative_promoter_anchor) ~
        "shorter_TSS_to_anchor_midpoint_distance",
      TRUE ~ "unresolved_equal_evidence_and_distance"
    ),
    representative_distance_advantage_bp = abs(
      min_tss_to_anchor_midpoint_distance_bp_anchor1 -
        min_tss_to_anchor_midpoint_distance_bp_anchor2
    ),
    representative_anchor_selection_status = if_else(
      is.na(representative_promoter_anchor),
      "unresolved_tie",
      "selected_for_gene_centric_sensitivity_only"
    ),
...
df.dual.primary.anchor.representative.summary <-
  df.dual.primary.anchor.representative.score %>%
  count(
    representative_anchor_selection_reason,
    representative_anchor_selection_status,
    name = "n_loops"
  ) %>%
  mutate(
    pct_dual_primary_loops = round(
      100 * n_loops /
        nrow(df.dual.primary.anchor.representative.score),
      1
    )
  ) %>%
...
df.dual.primary.representative.gene.assignment <-
  ...
  inner_join(
    df.dual.primary.anchor.representative.score %>%
      ...
      transmute(
        loop_id,
        anchor_side = representative_promoter_anchor,
        representative_anchor_selection_reason,
        representative_distance_advantage_bp
      ),
    by = c("loop_id", "anchor_side")
  ) %>%
...
assert_analysis_row_count(
  df.dual.primary.anchor.representative.score,
  sum(
    df.direct.promoter.tss.loop.summary$n_direct_anchor_sides == 2L
  ),
...
df.secondary.inward.proximal.loop.summary <-
  df.proximal.promoter.tss.loop.summary %>%
  dplyr::select(
    loop_id,
    resolution,
...
df.secondary.inward.proximal.by.resolution <- bind_rows(
  ...
  summarise(
    ...
    median_midpoint_nearest_inward_edge_distance_bp = median(
      min_midpoint_nearest_inward_proximal_distance_bp
    ),
    .groups = "drop"
  )
...
assert_analysis_condition(
  nrow(df.proximal.promoter.tss.loop.summary) ==
    nrow(df.loop.universe) &&
    !any(is.na(
      df.proximal.promoter.tss.loop.summary$n_proximal_anchor_sides
    )),
  "Proximal loop summary failed to preserve the pooled loop universe."
)

df.proximal.promoter.tss.by.resolution.distance <-
  df.proximal.promoter.tss.anchor.pair %>%
...
proximal.distance.thresholds.bp <- c(
  1000L,
  5000L,
  10000L,
  25000L,
...
direct.supported.loop.ids <-
  df.direct.promoter.tss.loop.summary %>%
  filter(has_any_direct_promoter_tss) %>%
  pull(loop_id)

df.proximal.promoter.tss.cumulative.threshold.summary <-
  map_dfr(proximal.distance.thresholds.bp, function(distance.threshold) {
    df.threshold <- df.proximal.promoter.tss.anchor.pair %>%
      filter(
        anchor_annotation_edge_distance_bp <= distance.threshold
      )
    df.threshold.inward <- df.threshold %>%
      filter(is_inward_proximal_evidence)
    df.threshold.inward.resolved <- df.threshold %>%
      filter(is_midpoint_nearest_inward_proximal_evidence)
    proximal.loop.ids <- unique(df.threshold$loop_id)
    inward.loop.ids <- unique(df.threshold.inward$loop_id)
    inward.resolved.loop.ids <- unique(
      df.threshold.inward.resolved$loop_id
    )

    tibble(
      max_anchor_edge_distance_bp = distance.threshold,
      n_annotation_records = nrow(df.threshold),
      n_loop_anchor_pairs = n_distinct(
        df.threshold$loop_id,
        df.threshold$anchor_side
      ),
      n_loop_anchor_gene_assignments = n_distinct(
        df.threshold$loop_id,
        df.threshold$anchor_side,
        df.threshold$gene_id
      ),
      n_loops_with_proximity = length(proximal.loop.ids),
      n_inward_annotation_records = nrow(df.threshold.inward),
      n_inward_loop_anchor_pairs = n_distinct(
        df.threshold.inward$loop_id,
        df.threshold.inward$anchor_side
      ),
      n_loops_with_inward_proximity = length(inward.loop.ids),
      n_midpoint_nearest_inward_annotation_records = nrow(
        df.threshold.inward.resolved
      ),
      n_midpoint_nearest_inward_loop_anchor_pairs = n_distinct(
        df.threshold.inward.resolved$loop_id,
        df.threshold.inward.resolved$anchor_side
      ),
      n_loops_with_midpoint_nearest_inward_proximity = length(
        inward.resolved.loop.ids
      ),
      pct_pooled_loops_with_proximity = round(
        100 * length(proximal.loop.ids) / nrow(df.loop.universe),
        1
      ),
      n_direct_and_proximity_loops = length(
        intersect(proximal.loop.ids, direct.supported.loop.ids)
      ),
      n_proximity_without_direct_loops = length(
        setdiff(proximal.loop.ids, direct.supported.loop.ids)
      ),
      n_direct_without_proximity_loops = length(
        setdiff(direct.supported.loop.ids, proximal.loop.ids)
      ),
      n_without_direct_or_proximity_loops = nrow(df.loop.universe) -
        length(union(proximal.loop.ids, direct.supported.loop.ids))
    )
  })
...
df.proximal.promoter.tss.summary <- tibble(
  ...
  n = c(
    ...
    sum(
      df.proximal.promoter.tss.loop.summary$n_proximal_anchor_sides == 1L
    ),
    sum(
      df.proximal.promoter.tss.loop.summary$
        has_proximal_promoter_tss_both_anchors
    ...
    sum(
      df.proximal.promoter.tss.loop.summary$
        promoter_tss_assignment_tier == "direct_only"
    ),
    sum(
      df.proximal.promoter.tss.loop.summary$
        promoter_tss_assignment_tier == "secondary_inward_10kb_only"
    ),
    sum(
      df.proximal.promoter.tss.loop.summary$
        promoter_tss_assignment_tier == "exploratory_proximal_200kb_only"
    ),
    sum(
      df.proximal.promoter.tss.loop.summary$
        promoter_tss_assignment_tier == "no_direct_or_proximal_evidence"
    )
  )
)
...
message(
  ...
  " loop-anchor-gene assignments across ",
  sum(
    df.proximal.promoter.tss.loop.summary$has_any_proximal_promoter_tss
  ),
  " pooled loops."
)
message(
...
df.revised.promoter.window.atac.record <-
  df.direct.promoter.tss.anchor.overlap %>%
  distinct(
    loop_id,
    resolution,
...
gr.revised.promoter.window.atac.record <- GRanges(
  ...
  ranges = IRanges(
    start = df.revised.promoter.window.atac.record$annotation_start,
    end = df.revised.promoter.window.atac.record$annotation_end
  ),
...
df.revised.promoter.window.atac.overlap <-
  summarise_anchor_interval_overlap(
...
assert_analysis_condition(
  nrow(df.revised.promoter.window.atac.anchor.summary) ==
    sum(
      df.direct.promoter.tss.anchor.summary$
        has_any_direct_promoter_tss
    ) &&
    !anyDuplicated(
      str_c(
        df.revised.promoter.window.atac.anchor.summary$loop_id,
        df.revised.promoter.window.atac.anchor.summary$anchor_side,
        sep = "|"
      )
    ),
...
gr.loop.anchor.all <- combine_loop_anchor_granges_by_side(
  gr.loop.anchor.by.side
)
...
df.revised.atac.anchor.evidence <-
  df.revised.raw.atac.anchor.overlap %>%
  transmute(
    anchor_index,
    loop_id,
    resolution,
    ...
    raw_atac_n_intervals_ge50 =
      n_overlapping_feature_intervals_ge_minimum,
    raw_atac_overlap_bp = feature_overlap_bp,
    raw_atac_overlap_fraction_of_anchor =
      feature_overlap_fraction_of_anchor,
  ...
  left_join(
    df.revised.tss.exclusion.anchor.overlap %>%
      transmute(
        ...
        known_tss_exclusion_overlap_fraction_of_anchor =
          feature_overlap_fraction_of_anchor,
  ...
  mutate(
    ...
    non_tss_atac_overlap_fraction_of_non_tss_anchor = if_else(
      has_non_tss_anchor_fragment,
      non_tss_atac_overlap_bp / non_tss_anchor_bp,
      NA_real_
    )
  ) %>%
  left_join(
  ...
  mutate(
    n_direct_promoter_windows = coalesce(
      n_direct_promoter_windows,
      0L
    ),
    n_atac_positive_promoter_windows = coalesce(
      n_atac_positive_promoter_windows,
      0L
    ),
    promoter_window_atac_overlap_any = coalesce(
      promoter_window_atac_overlap_any,
      FALSE
    ),
    promoter_window_atac_overlap_ge50 = coalesce(
      promoter_window_atac_overlap_ge50,
      FALSE
    ),
    promoter_window_atac_max_overlap_bp = coalesce(
...
assert_analysis_condition(
  nrow(df.revised.atac.anchor.evidence) ==
    2L * nrow(df.loop.universe) &&
    !anyDuplicated(
      str_c(
        df.revised.atac.anchor.evidence$loop_id,
        df.revised.atac.anchor.evidence$anchor_side,
        sep = "|"
      )
    ) &&
...
df.revised.atac.promoter.anchor.metric <-
  df.revised.atac.anchor.evidence %>%
  transmute(
    loop_id,
    ...
    promoter_window_atac_overlap_ge50,
    promoter_window_atac_max_overlap_bp,
    promoter_n_atac_positive_windows =
      n_atac_positive_promoter_windows,
    ...
    promoter_known_tss_exclusion_overlap_bp =
      known_tss_exclusion_overlap_bp,
    promoter_non_tss_anchor_bp = non_tss_anchor_bp
  )

df.revised.atac.candidate.anchor.metric <-
  df.revised.atac.anchor.evidence %>%
  transmute(
    loop_id,
    ...
    candidate_anchor_has_direct_promoter_tss =
      has_any_direct_promoter_tss,
    candidate_promoter_window_atac_overlap_any =
      promoter_window_atac_overlap_any,
    candidate_promoter_window_atac_overlap_ge50 =
      promoter_window_atac_overlap_ge50,
    candidate_promoter_window_atac_max_overlap_bp =
      promoter_window_atac_max_overlap_bp,
  ...
  left_join(
    df.direct.promoter.tss.loop.summary %>%
      dplyr::select(
        loop_id,
        n_direct_anchor_sides,
        direct_anchor_assignment_class
      ),
    by = "loop_id"
  ) %>%
  left_join(
    df.revised.atac.promoter.anchor.metric,
    by = c("loop_id", "promoter_anchor_side")
  ) %>%
  left_join(
    df.revised.atac.candidate.anchor.metric,
    by = c("loop_id", "candidate_anchor_side")
  ) %>%
  mutate(
    is_unambiguous_candidate_regulatory_orientation =
      n_direct_anchor_sides == 1L,
    directional_mixed_regulatory_support = (
      promoter_window_atac_overlap_ge50 &
        candidate_non_tss_atac_overlap_ge50
    ),
    candidate_open_chromatin_support_class = case_when(
      !is_unambiguous_candidate_regulatory_orientation ~
        "both_anchors_direct_promoter_TSS_no_unique_candidate",
      candidate_non_tss_atac_overlap_ge50 ~
        "opposite_anchor_nonTSS_ATAC_ge50",
      candidate_non_tss_atac_overlap_any ~
        "opposite_anchor_nonTSS_ATAC_lt50_only",
      !candidate_has_non_tss_anchor_fragment ~
        "opposite_anchor_fully_within_known_TSS_exclusion",
...
df.dual.promoter.atac.directional.classification <-
  ...
  summarise(
    promoter_window_atac_ge50_anchor1 = any(
      promoter_anchor_side == "anchor1" &
        promoter_window_atac_overlap_ge50
    ),
    promoter_window_atac_ge50_anchor2 = any(
      promoter_anchor_side == "anchor2" &
        promoter_window_atac_overlap_ge50
    ),
    residual_non_tss_atac_ge50_anchor1 = any(
      candidate_anchor_side == "anchor1" &
        candidate_non_tss_atac_overlap_ge50
    ),
    residual_non_tss_atac_ge50_anchor2 = any(
      candidate_anchor_side == "anchor2" &
        candidate_non_tss_atac_overlap_ge50
    ),
    supports_anchor1_promoter_to_anchor2_regulatory = any(
      promoter_anchor_side == "anchor1" &
        directional_mixed_regulatory_support
    ),
    supports_anchor2_promoter_to_anchor1_regulatory = any(
      promoter_anchor_side == "anchor2" &
        directional_mixed_regulatory_support
    ),
    .groups = "drop"
  ...
  mutate(
    ...
    supported_regulatory_direction = case_when(
      supports_anchor1_promoter_to_anchor2_regulatory &
        !supports_anchor2_promoter_to_anchor1_regulatory ~
        "anchor1_promoter_to_anchor2_regulatory",
      !supports_anchor1_promoter_to_anchor2_regulatory &
        supports_anchor2_promoter_to_anchor1_regulatory ~
        "anchor2_promoter_to_anchor1_regulatory",
      supports_anchor1_promoter_to_anchor2_regulatory &
        supports_anchor2_promoter_to_anchor1_regulatory ~
        "both_directions_supported",
...
assert_analysis_condition(
  nrow(df.dual.promoter.atac.directional.classification) ==
    ...
    sum(df.dual.promoter.atac.directional.summary$n_loops) ==
      nrow(df.dual.promoter.atac.directional.classification) &&
    !anyDuplicated(
      df.dual.promoter.atac.directional.classification$loop_id
    ),
...
assert_analysis_condition(
  nrow(df.revised.atac.unambiguous.orientation) ==
    sum(df.direct.promoter.tss.loop.summary$n_direct_anchor_sides == 1L) &&
...
df.revised.atac.anchor.wide <-
  df.revised.atac.anchor.evidence %>%
  dplyr::select(
    loop_id,
    anchor_side,
    raw_atac_overlap_any,
    raw_atac_overlap_ge50,
    raw_atac_overlap_bp,
    non_tss_atac_overlap_any,
    non_tss_atac_overlap_ge50,
    non_tss_atac_overlap_bp,
    known_tss_exclusion_overlap_bp,
    non_tss_anchor_bp
  ) %>%
  pivot_wider(
  ...
  mutate(
    promoter_anchor_raw_atac_ge50 = case_when(
      direct_anchor_assignment_class == "direct_anchor1_only" ~
        raw_atac_overlap_ge50_anchor1,
      direct_anchor_assignment_class == "direct_anchor2_only" ~
        raw_atac_overlap_ge50_anchor2,
      TRUE ~ NA
    ),
    ...
    candidate_anchor_non_tss_atac_any = case_when(
      direct_anchor_assignment_class == "direct_anchor1_only" ~
        non_tss_atac_overlap_any_anchor2,
      direct_anchor_assignment_class == "direct_anchor2_only" ~
        non_tss_atac_overlap_any_anchor1,
      TRUE ~ NA
    ),
    candidate_anchor_non_tss_atac_ge50 = case_when(
      direct_anchor_assignment_class == "direct_anchor1_only" ~
        non_tss_atac_overlap_ge50_anchor2,
      direct_anchor_assignment_class == "direct_anchor2_only" ~
        non_tss_atac_overlap_ge50_anchor1,
      TRUE ~ NA
    ),
    revised_atac_evidence_class = case_when(
      n_direct_anchor_sides == 2L ~
        "direct_promoter_TSS_both_anchors",
      n_direct_anchor_sides == 1L & candidate_anchor_non_tss_atac_ge50 ~
        "single_direct_promoter_TSS_opposite_nonTSS_ATAC_ge50",
      n_direct_anchor_sides == 1L & candidate_anchor_non_tss_atac_any ~
        "single_direct_promoter_TSS_opposite_nonTSS_ATAC_lt50_only",
      n_direct_anchor_sides == 1L ~
        "single_direct_promoter_TSS_no_opposite_nonTSS_ATAC",
...
assert_analysis_row_count(
  df.revised.atac.loop.summary,
  nrow(df.loop.universe),
  "Revised ATAC loop summary lost pooled loops."
)
...
df.revised.atac.support.by.resolution <-
  ...
  summarise(
    ...
    pct_promoter_raw_atac_any = round(
      100 * mean(promoter_raw_atac_overlap_any),
      1
    ),
    n_promoter_raw_atac_ge50 = sum(promoter_raw_atac_overlap_ge50),
    pct_promoter_raw_atac_ge50 = round(
      100 * mean(promoter_raw_atac_overlap_ge50),
      1
    ),
    n_opposite_raw_atac_any = sum(candidate_raw_atac_overlap_any),
    pct_opposite_raw_atac_any = round(
      100 * mean(candidate_raw_atac_overlap_any),
      1
    ),
    n_opposite_raw_atac_ge50 = sum(candidate_raw_atac_overlap_ge50),
    pct_opposite_raw_atac_ge50 = round(
      100 * mean(candidate_raw_atac_overlap_ge50),
      1
    ),
    n_opposite_non_tss_atac_any = sum(
      candidate_non_tss_atac_overlap_any
    ),
    pct_opposite_non_tss_atac_any = round(
      100 * mean(candidate_non_tss_atac_overlap_any),
      1
    ),
    n_opposite_non_tss_atac_ge50 = sum(
      candidate_non_tss_atac_overlap_ge50
    ),
    pct_opposite_non_tss_atac_ge50 = round(
      100 * mean(candidate_non_tss_atac_overlap_ge50),
      1
    ),
    .groups = "drop"
  ) %>%
...
df.revised.atac.unambiguous.candidate.anchor <-
  df.revised.atac.unambiguous.orientation %>%
  transmute(
    loop_id,
    resolution,
    ...
    per_anchor_non_tss_atac_overlap_bp =
      candidate_non_tss_atac_overlap_bp
  )

gr.revised.atac.unambiguous.candidate.anchor <- GRanges(
  seqnames = df.revised.atac.unambiguous.candidate.anchor$anchor_chr,
  ranges = IRanges(
    start = df.revised.atac.unambiguous.candidate.anchor$anchor_start,
    end = df.revised.atac.unambiguous.candidate.anchor$anchor_end
  ),
...
df.revised.atac.candidate.non.tss.fragment <-
  subtract_exclusion_from_anchor_ranges(
    gr.revised.atac.unambiguous.candidate.anchor,
    gr.known.tss.exclusion
  ) %>%
...
gr.revised.atac.candidate.non.tss.fragment <- GRanges(
  ...
  ranges = IRanges(
    start = df.revised.atac.candidate.non.tss.fragment$fragment_start,
    end = df.revised.atac.candidate.non.tss.fragment$fragment_end
  ),
...
df.revised.atac.candidate.non.tss.fragment.evidence <-
  df.revised.atac.candidate.non.tss.fragment %>%
  left_join(
    df.revised.atac.fragment.overlap %>%
      transmute(
        ...
        fragment_atac_n_intervals_ge50 =
          n_overlapping_feature_intervals_ge_minimum,
        fragment_atac_overlap_bp = feature_overlap_bp,
        fragment_atac_overlap_fraction =
          feature_overlap_fraction_of_anchor,
...
df.revised.atac.fragment.anchor.observed <-
  df.revised.atac.candidate.non.tss.fragment.evidence %>%
  group_by(loop_id, resolution, anchor_side) %>%
  summarise(
...
df.revised.atac.fragment.anchor.summary <-
  ...
  mutate(
    n_non_tss_anchor_fragments = coalesce(
      n_non_tss_anchor_fragments,
      0L
    ),
    fragment_non_tss_anchor_bp = coalesce(
      fragment_non_tss_anchor_bp,
      0L
    ),
    fragment_non_tss_atac_overlap_bp = coalesce(
      fragment_non_tss_atac_overlap_bp,
      0L
    ),
    fragment_non_tss_atac_any = coalesce(
      fragment_non_tss_atac_any,
      FALSE
    ),
    fragment_non_tss_atac_ge50 = coalesce(
      fragment_non_tss_atac_ge50,
      FALSE
    ),
    fragment_vs_anchor_ge50_status = case_when(
      per_anchor_non_tss_atac_ge50 & fragment_non_tss_atac_ge50 ~
        "positive_by_both_methods",
...
assert_analysis_condition(
  !any(
    df.revised.atac.fragment.anchor.summary$per_anchor_non_tss_bp !=
      df.revised.atac.fragment.anchor.summary$fragment_non_tss_anchor_bp
  ),
  "Fragment-level TSS subtraction disagrees with per-anchor residual bases."
)

df.revised.atac.method.comparison.summary <-
  bind_rows(
    df.revised.atac.fragment.anchor.summary,
    df.revised.atac.fragment.anchor.summary %>%
      mutate(resolution = "ALL")
  ...
  complete(
    resolution,
    fragment_vs_anchor_ge50_status = c(
      "positive_by_both_methods",
      "per_anchor_only",
      "fragment_only",
      "negative_by_both_methods"
    ),
...
df.direct.assignment.for.position <-
  df.direct.promoter.tss.gene.assignment %>%
  mutate(
...
df.proximal.assignment.for.position <-
  df.proximal.promoter.tss.gene.assignment %>%
  mutate(
...
df.direct.transcript.position.detail <-
  annotate_assignment_transcript_positions(
...
df.proximal.transcript.position.detail <-
  annotate_assignment_transcript_positions(
...
df.direct.gene.assignment.position.flags <-
  summarise_assignment_transcript_positions(
...
df.transcript.position.summary <- bind_rows(
  ...
  summarise(
    n_loop_anchor_gene_assignments = n(),
    n_with_ensembl_transcript_annotation = sum(
      has_ensembl_transcript_annotation
    ),
    pct_with_ensembl_transcript_annotation = round(
      100 * mean(has_ensembl_transcript_annotation),
      1
    ),
    n_any_transcript_fully_within_loop_span = sum(
      any_transcript_fully_within_loop_span
    ),
    pct_any_transcript_fully_within_loop_span = round(
      100 * mean(any_transcript_fully_within_loop_span),
      1
    ),
    n_all_transcripts_fully_within_loop_span = sum(
      all_transcripts_fully_within_loop_span
    ),
    pct_all_transcripts_fully_within_loop_span = round(
      100 * mean(all_transcripts_fully_within_loop_span),
      1
    ),
    n_any_transcript_fully_within_inter_anchor_interval = sum(
      any_transcript_fully_within_inter_anchor_interval
    ),
    pct_any_transcript_fully_within_inter_anchor_interval = round(
      100 * mean(any_transcript_fully_within_inter_anchor_interval),
      1
    ),
    n_overlap_without_full_transcript_containment = sum(
      transcript_containment_summary ==
        "overlap_without_full_transcript_containment"
    ),
    n_all_annotated_transcripts_outside_loop_span = sum(
      transcript_containment_summary ==
        "all_annotated_transcripts_outside_loop_span"
    ),
    .groups = "drop"
  ) %>%
...
df.ctcf.evidence <- df.loop.universe %>%
  ...
  transmute(
    loop_id,
    resolution,
    predicted_ctcf_motif_interval_count_anchor1 = coalesce(
      ctcf_count_anchor1,
      0L
    ),
    predicted_ctcf_motif_interval_count_anchor2 = coalesce(
      ctcf_count_anchor2,
      0L
    )
  ) %>%
  mutate(
    predicted_ctcf_motif_intervals_any_anchor1 =
      predicted_ctcf_motif_interval_count_anchor1 > 0L,
    predicted_ctcf_motif_intervals_any_anchor2 =
      predicted_ctcf_motif_interval_count_anchor2 > 0L,
    predicted_ctcf_motif_intervals_both_anchors = (
      predicted_ctcf_motif_intervals_any_anchor1 &
        predicted_ctcf_motif_intervals_any_anchor2
    ),
    predicted_ctcf_motif_annotation_class = case_when(
      predicted_ctcf_motif_intervals_both_anchors ~
        "predicted_motif_intervals_at_both_anchors",
      predicted_ctcf_motif_intervals_any_anchor1 |
        predicted_ctcf_motif_intervals_any_anchor2 ~
        "predicted_motif_intervals_at_one_anchor",
...
df.revised.loop.evidence <- df.loop.universe %>%
  ...
  left_join(
    df.revised.atac.loop.summary %>%
      dplyr::select(
        loop_id,
        promoter_anchor_raw_atac_ge50,
        candidate_anchor_raw_atac_ge50,
        candidate_anchor_non_tss_atac_any,
        candidate_anchor_non_tss_atac_ge50,
        revised_atac_evidence_class
      ),
    by = "loop_id"
  ) %>%
  left_join(
  ...
  mutate(
    ...
    dual_promoter_directional_category = replace_na(
      dual_promoter_directional_category,
      "not_dual_promoter_loop"
    ),
    supported_regulatory_direction = replace_na(
      supported_regulatory_direction,
      "not_dual_promoter_loop"
    ),
    ...
    revised_detailed_category = case_when(
      revised_putative_regulatory_support ~
        "single_direct_promoter_TSS_with_opposite_nonTSS_ATAC",
      revised_promoter_promoter_compatible ~ str_c(
        "promoter_promoter_compatible__",
        dual_promoter_directional_category
      ),
      revised_single_promoter_without_opposite_atac ~
        "single_direct_promoter_TSS_without_opposite_nonTSS_ATAC",
      has_any_secondary_inward_proximal_10kb ~
        "no_direct_promoter_TSS_secondary_inward_10kb",
      has_any_proximal_promoter_tss ~
        "no_direct_promoter_TSS_exploratory_proximal_200kb_only",
...
df.revised.loop.gene.resource <-
  ...
  mutate(
    ...
    revised_gene_assignment_role = case_when(
      revised_putative_regulatory_support ~
        "direct_promoter_TSS_gene_in_putative_regulatory_loop",
      revised_promoter_promoter_compatible & anchor_side == "anchor1" ~
        "promoter_TSS_gene_at_anchor1_in_promoter_promoter_loop",
      revised_promoter_promoter_compatible & anchor_side == "anchor2" ~
        "promoter_TSS_gene_at_anchor2_in_promoter_promoter_loop",
      revised_single_promoter_without_opposite_atac ~
        "direct_promoter_TSS_gene_without_opposite_ATAC_support",
...
assert_analysis_condition(
  nrow(df.revised.loop.gene.resource) ==
    nrow(df.direct.promoter.tss.gene.assignment) &&
...
assert_analysis_condition(
  n_distinct(
    df.revised.loop.gene.resource$loop_id[
      df.revised.loop.gene.resource$revised_putative_regulatory_support
    ]
  ) == sum(df.revised.loop.evidence$revised_putative_regulatory_support),
...
df.revised.secondary.loop.gene.resource <-
  ...
  mutate(
    ensembl_gene_id = gene_id,
    gene_symbol = gene_name,
    revised_gene_assignment_role =
      "secondary_inward_proximal_10kb_promoter_TSS_candidate"
  ) %>%
  arrange(loop_id, anchor_side, ensembl_gene_id)
...
df.revised.gene.count.by.set <-
  summarise_revised_gene_loop_counts(
    df.revised.gene.loop.membership
  )
...
df.approach2.loop.gene.input <- df.revised.gene.loop.membership %>%
  left_join(
    df.loop.universe %>%
      dplyr::select(loop_id, loop_distance),
    by = "loop_id"
  ) %>%
  transmute(
    gene_set,
...
approach2.gene.set.names <- sort(
  unique(df.approach2.loop.gene.input$gene_set)
)
approach2.results <- set_names(
  map(approach2.gene.set.names, function(gene.set.i) {
    approach_2nd_analyze_loops_by_threshold(
      df = df.approach2.loop.gene.input %>%
        filter(gene_set == gene.set.i) %>%
        dplyr::select(gene_id, loop.id, distance),
...
df.approach2.gene.loop.count <- imap_dfr(
  approach2.results,
  function(result.i, gene.set.i) {
    result.i$gene_loop_count %>%
      mutate(gene_set = gene.set.i, .before = 1)
  }
) %>%
  left_join(
    df.approach2.gene.symbol.lookup,
    by = c("gene_set", "gene_id")
  ) %>%
  arrange(gene_set, desc(n), gene_symbol, gene_id)

df.approach2.multiple.interaction.genes <- imap_dfr(
  approach2.results,
  function(result.i, gene.set.i) {
    result.i$multiple_interaction_genes %>%
      mutate(gene_set = gene.set.i, .before = 1)
  }
) %>%
  left_join(
...
df.approach2.multiple.interaction.summary <- imap_dfr(
  approach2.results,
  function(result.i, gene.set.i) {
    result.i$summary %>%
      mutate(gene_set = gene.set.i, .before = 1)
...
df.revised.gene.count.threshold.summary <-
  df.revised.gene.count.by.set %>%
  group_by(gene_set) %>%
  summarise(
    n_genes = n(),
...
revised.resource.tables <- list(
  ...
  revised_secondary_inward_10kb_loop_gene_assignments =
    df.revised.secondary.loop.gene.resource,
  revised_secondary_inward_10kb_loops =
    df.secondary.inward.proximal.loop.summary %>%
      filter(has_any_secondary_inward_proximal_10kb),
  revised_putative_regulatory_loops = df.revised.loop.evidence %>%
    filter(revised_putative_regulatory_support),
  revised_promoter_promoter_compatible_loops =
    df.revised.loop.evidence %>%
      filter(revised_promoter_promoter_compatible),
  revised_single_promoter_without_opposite_atac_loops =
    df.revised.loop.evidence %>%
      filter(revised_single_promoter_without_opposite_atac),
...
df.canonical.tss.main.loop.gene.membership <-
  ...
  transmute(
    ...
    transcript_id,
    transcript_id_versioned,
    true_tss_id = str_remove(annotation_id, ":TSS_pm1kb$")
  ) %>%
  distinct(loop_id, ensembl_gene_id, .keep_all = TRUE) %>%
...
revised.go.gene.sets$revised_putative_canonical_TSS_only <-
  df.canonical.tss.main.loop.gene.membership %>%
...
revised.go.canonical.universe <-
  df.primary.direct.true.tss.anchor.overlap %>%
  filter(
    coalesce(is_ensembl_canonical, FALSE),
    !is.na(gene_id)
  ) %>%
  distinct(gene_id) %>%
  pull(gene_id)
...
df.revised.go.significance.summary <-
  df.revised.go.input.summary %>%
  left_join(
    df.revised.go.result %>%
      group_by(gene_set) %>%
      summarise(
        n_GO_BP_terms_tested = n(),
        n_GO_BP_terms_FDR_lt_0_05 = sum(p.adjust < 0.05, na.rm = TRUE),
        minimum_adjusted_p = min(p.adjust, na.rm = TRUE),
        .groups = "drop"
      ),
    by = "gene_set"
  ) %>%
  mutate(
    ...
    n_GO_BP_terms_FDR_lt_0_05 = coalesce(
      n_GO_BP_terms_FDR_lt_0_05,
      0L
    )
  ) %>%
  arrange(gene_set)
...
df.main.all.annotation.gene.loop.locus.count <-
  summarise_gene_loop_locus_counts(
    df.main.all.annotation.gene.loop.membership,
    df.approximate.loop.locus.map,
    evidence.definition = "all_transcript_TSS_plus_EPD_primary_assignment"
  )
df.main.canonical.tss.gene.loop.locus.count <-
  summarise_gene_loop_locus_counts(
    df.canonical.tss.main.loop.gene.membership,
    df.approximate.loop.locus.map,
    evidence.definition = "Ensembl_canonical_transcript_TSS_only"
  )
...
df.gene.rank.sensitivity.detail <-
  df.main.all.annotation.gene.loop.locus.count %>%
  transmute(
    ensembl_gene_id,
    all_annotation_gene_symbol = gene_symbol,
    all_annotation_n_exact_loop_calls = n_exact_loop_calls,
    all_annotation_n_approximate_loop_loci = n_approximate_loop_loci
  ) %>%
  full_join(
    df.main.canonical.tss.gene.loop.locus.count %>%
      transmute(
        ensembl_gene_id,
        canonical_tss_gene_symbol = gene_symbol,
        canonical_tss_n_exact_loop_calls = n_exact_loop_calls,
        canonical_tss_n_approximate_loop_loci = n_approximate_loop_loci
      ),
    by = "ensembl_gene_id"
  ) %>%
  mutate(
    gene_symbol = coalesce(
      all_annotation_gene_symbol,
      canonical_tss_gene_symbol
    ),
    across(ends_with("loop_calls"), ~ replace_na(.x, 0L)),
    across(ends_with("loop_loci"), ~ replace_na(.x, 0L)),
    rank_all_annotation_exact_calls = min_rank(
      dplyr::desc(all_annotation_n_exact_loop_calls)
    ),
    rank_all_annotation_approximate_loci = min_rank(
      dplyr::desc(all_annotation_n_approximate_loop_loci)
    ),
    rank_canonical_tss_exact_calls = min_rank(
      dplyr::desc(canonical_tss_n_exact_loop_calls)
    ),
    rank_canonical_tss_approximate_loci = min_rank(
      dplyr::desc(canonical_tss_n_approximate_loop_loci)
    )
  ) %>%
  dplyr::select(
    ensembl_gene_id,
    gene_symbol,
    all_annotation_n_exact_loop_calls,
    all_annotation_n_approximate_loop_loci,
    canonical_tss_n_exact_loop_calls,
    canonical_tss_n_approximate_loop_loci,
    starts_with("rank_")
  ) %>%
  arrange(rank_all_annotation_exact_calls, ensembl_gene_id)

df.gene.rank.sensitivity.correlation <- bind_rows(
  summarise_gene_count_spearman(
    df.gene.rank.sensitivity.detail,
    "all_annotation_n_exact_loop_calls",
    "all_annotation_n_approximate_loop_loci",
    "all_annotation_exact_calls_vs_approximate_loci"
  ),
  summarise_gene_count_spearman(
    df.gene.rank.sensitivity.detail,
    "all_annotation_n_exact_loop_calls",
    "canonical_tss_n_exact_loop_calls",
    "all_annotation_vs_canonical_TSS_exact_calls"
  ),
  summarise_gene_count_spearman(
    df.gene.rank.sensitivity.detail,
    "all_annotation_n_approximate_loop_loci",
...
df.figure5.loop.window <- df.loop.universe %>%
  transmute(
    ...
    resolution = factor(
      resolution,
      levels = names(figure5.resolution.colors)
    ),
...
gr.figure5.loop.window <- GRanges(
  ...
  ranges = IRanges(
    start = df.figure5.loop.window$expanded_start,
...
df.figure5.true.tss.site <- df.true.tss.transcript %>%
  ...
  mutate(
    feature_id = str_c(chr, true_tss_start, strand, sep = ":"),
    feature_position = as.numeric(true_tss_start),
...
gr.figure5.true.tss.site <- GRanges(
  ...
  ranges = IRanges(
    start = df.figure5.true.tss.site$true_tss_start,
    end = df.figure5.true.tss.site$true_tss_end
  )
)
...
figure5.true.tss.hit <- findOverlaps(
  gr.figure5.true.tss.site,
  gr.figure5.loop.window,
  type = "any",
  select = "all"
)
df.figure5.true.tss.relative.position <- tibble(
  ...
  transmute(
    ...
    feature_position =
      df.figure5.true.tss.site$feature_position[feature_index],
    feature_strand = df.figure5.true.tss.site$strand[feature_index],
    anchor1_midpoint =
      df.figure5.loop.window$anchor1_midpoint[loop_index],
    anchor2_midpoint =
      df.figure5.loop.window$anchor2_midpoint[loop_index],
    anchor_midpoint_distance =
      df.figure5.loop.window$anchor_midpoint_distance[loop_index],
    relative_position =
      (feature_position - anchor1_midpoint) / anchor_midpoint_distance,
    feature_type = "strand_aware_true_TSS"
  ) %>%
...
df.figure5.promoter.site <-
  ...
  transmute(
    feature_id = promoter_annotation_id,
    chr,
    promoter_start,
    promoter_end,
    strand,
...
gr.figure5.promoter.site <- GRanges(
  ...
  ranges = IRanges(
    start = df.figure5.promoter.site$promoter_start,
    end = df.figure5.promoter.site$promoter_end
  )
)
...
figure5.promoter.hit <- findOverlaps(
  gr.figure5.promoter.site,
  gr.figure5.loop.window,
  type = "any",
  select = "all"
)
df.figure5.promoter.relative.position <- tibble(
  ...
  transmute(
    ...
    feature_position =
      df.figure5.promoter.site$feature_position[feature_index],
...
df.source.data.versions <- bind_rows(
  ...
  mutate(
    ...
    modified_time = if_else(
      file_exists,
      format(file.info(input_path)$mtime, "%Y-%m-%d %H:%M:%S %Z"),
      NA_character_
    ),
    sha256 = map_chr(input_path, sha256_file),
    genome_assembly = case_when(
      input_group == "analysis_code" ~ NA_character_,
      input_name %in% c("epd_rn6_bed", "epd_coordinate") ~
        "rn6 source; normalized to rn7",
      TRUE ~ "mRatBN7.2/rn7"
    ),
    provenance_note = case_when(
      str_starts(input_name, "hiccups_loop_") ~
        "Juicer 1.6; Juicer Tools/HiCCUPS 1.22.01 source BEDPE",
      input_name == "ctcf_motif" ~
        "Full FIMO output from 100 PWM models; predicted motif intervals",
      input_name == "atac_peak" ~
        "Duttke et al. 2022 rat PFC snATAC peaks normalized to rn7",
      input_name == "ensembl_gtf" ~
        "Ensembl Rattus_norvegicus mRatBN7.2 release 113 GTF",
      str_starts(input_name, "epd_") ~
        "EPDnew promoter source used for strand-aware rn6-to-rn7 reconstruction",
      input_name == "library_complexity" ~
        "Hi-C usable-contact depth QC input",
...
assert_analysis_condition(
  all(df.source.data.versions$file_exists) &&
    !any(is.na(df.source.data.versions$sha256)),
...
atac.matched.null.output.files <- c(
  "revised_atac_threshold_sensitivity_by_resolution.tsv",
  "revised_atac_matched_null_summary.tsv",
  "revised_atac_matched_null_permutation.tsv",
  "revised_atac_matched_null_method.tsv",
  "revised_atac_matched_control_quality.tsv",
  "revised_atac_matched_control_strata.tsv",
  "revised_atac_matched_null_run_metadata.tsv",
  "revised_atac_observed_vs_matched_null.pdf",
  "revised_atac_observed_vs_matched_null.png",
  "revised_atac_matched_null_session_info.txt"
)
available.atac.matched.null.output.files <- intersect(
  atac.matched.null.output.files,
...
assert_analysis_condition(
  !anyDuplicated(generated.output.files),
  "The resubmission output manifest contains duplicate filenames."
)
...
protected.output.files <- c(
  generated.output.files,
  "resubmit_output_manifest.tsv"
)
stale.output.files <- setdiff(
  list.files(output.dir, all.files = FALSE, no.. = TRUE),
  protected.output.files
)
if (length(stale.output.files) > 0L) {
  stale.output.removed <- file.remove(
    file.path(output.dir, stale.output.files)
  )
  assert_analysis_condition(
    all(stale.output.removed),
    "One or more stale resubmission output files could not be removed."
  )
}
...
assert_analysis_condition(
  all(df.output.manifest$file_exists) &&
    !any(is.na(df.output.manifest$sha256)),
  "Output manifest contains a missing file or checksum."
)
...
message(
  "Revised direct promoter/TSS-supported loops: ",
  sum(df.direct.promoter.tss.loop.summary$has_any_direct_promoter_tss)
)
message(
  "Revised direct loop-anchor-gene assignments: ",
  nrow(df.direct.promoter.tss.gene.assignment)
)
message(
  "Revised proximal promoter/TSS-supported loops (1-200 kb): ",
  sum(
    df.proximal.promoter.tss.loop.summary$
      has_any_proximal_promoter_tss
  )
...
message(
  "Revised proximal loop-anchor-gene assignments: ",
  nrow(df.proximal.promoter.tss.gene.assignment)
)
message(
  "Revised secondary inward <=10-kb supported loops: ",
  sum(
    df.proximal.promoter.tss.loop.summary$
      has_any_secondary_inward_proximal_10kb
  )
...
message(
  "Revised secondary inward <=10-kb loop-anchor-gene assignments: ",
  nrow(df.secondary.inward.proximal.gene.assignment)
)
message(
  "Revised direct/proximal assignment, ATAC, transcript-position, and loop-",
  "category sections are complete. Legacy analyses are excluded from this ",
  "production workflow."
)
