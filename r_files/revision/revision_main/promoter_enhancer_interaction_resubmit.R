...
df.dual.primary.anchor.representative.score <-
  ...
  mutate(
    ...
    representative_distance_advantage_bp = abs(
      min_tss_to_anchor_midpoint_distance_bp_anchor1 - min_tss_to_anchor_midpoint_distance_bp_anchor2
    ),
...
df.revised.atac.candidate.non.tss.fragment <-
  subtract_exclusion_from_anchor_ranges(gr.revised.atac.unambiguous.candidate.anchor, gr.known.tss.exclusion) %>%
...
df.main.all.annotation.gene.loop.locus.count <-
  summarise_gene_loop_locus_counts(df.main.all.annotation.gene.loop.membership, df.approximate.loop.locus.map, evidence.definition = "all_transcript_TSS_plus_EPD_primary_assignment")
df.main.canonical.tss.gene.loop.locus.count <-
  summarise_gene_loop_locus_counts(df.canonical.tss.main.loop.gene.membership, df.approximate.loop.locus.map, evidence.definition = "Ensembl_canonical_transcript_TSS_only")
