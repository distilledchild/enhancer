...
table.s2 <- df.loop.deep.sample.all %>% count(strain, resolution) %>% 
  pivot_wider(
...
df.init.loop.filtered.bed <- df.init.loop.bed %>% filter(
  fdrDonut < 0.01,
  fdrBL < 0.01,
  fdrH < 0.01,
  fdrV < 0.01,
  distance < 1e6,
  observed / expectedDonut > 2
) %>% # 44784
  mutate(end.distance = x2 - x1) %>% 
  mutate(loop.id = str_c(X.chr1, '_', x1, '_', x2, '_', chr2, '_', y1, '_', y2, '_', end.distance)) # loop.id

df.filtered.out.loop.bed <- anti_join(df.loop.deep.sample.all, df.init.loop.filtered.bed, by = 'loop.id' )
df.filtered.out.loop.bed %>% 
  # count(strain)
  count(resolution)
# resolution    n
# 1         5K 1862
...
line_graph_for_loops_per_depth <- ggplot(df_long_for_plot, aes(x = Depth, y = num_loop, color = Sequencing_Metric)) +
  # geom_line(aes(group = Sequencing_Metric), linewidth = 1.2) +
  geom_point() +
  ...
  labs(
    # title = "Number of Loop by Sequencing Depth",
    x = "Number of Reads",
    y = "Number of Loops",
    color = "Category"
  ) +
  geom_text(
    data = correlation_results_position, 
...
ggsave("figures/submission/line_graph_for_loops_per_depth.pdf", plot = line_graph_for_loops_per_depth, device = "pdf", width = 11*0.6, height = 8.5*0.6, units = "in")
...
chromosome_data <- read.table(file="/Users/PanjunKim/UTHSC\ GGI\ Dropbox/K\ P/Gateway_to_Hao/enhancer/data/rn7_chromosome_length.tsv", sep="\t", head=T) %>% 
...
df.init.ctcf<-read.table(file="/Users/PanjunKim/UTHSC GGI Dropbox/K P/Gateway_to_Hao/enhancer/data/ctcf/submission/E4/fimo_E4_submission_trial.txt", header=TRUE, sep="\t") %>% 
...
df.DISTINCT.fimo.2nd.trial.ctcf <- df.init.ctcf %>%
  distinct(chr, start, end) %>% # 2701585 | .4:3191859
...
pdf("./figures/submission/overall_distribution_of_CTCF_by_chromosome.pdf", width = 11*0.8, height = 8.5*0.8)
...
df.overlapping.CTCF.w.DOWNSTREAM.result %>% dim() # 1213237 | sub.4 any: 1411588 within: 1408116       7
...
df.overlapping.CTCF.w.BOTH.result %>% dim() # 2420002| sub.4 any: 2814516/ within: 2807418       
...
# GTF 
df.refgene.gtf <- read_tsv("/Users/PanjunKim/UTHSC GGI Dropbox/K P/Gateway_to_Hao/workshop/2023_NIH_meeting/loop_N_tss/ucsc_refGene.gtf",
                           comment = "#",  
...
df.tss.ucsc <- df.tss.ucsc.tss.id %>% distinct(chr, start, end) %>% # 17080******
  mutate(tss.id = paste(chr, start, end, sep = ":"))
df.tss.ucsc # 17080
...
df.DISTINCT.loop.deep.sample.all.padded.for.TSS <- df.DISTINCT.loop.deep.sample.all %>% 
  mutate(x12 = (x1 + x2)/2, # middle point of each end
         ...
         distance = y12 - x12,
         x0 = ifelse(x12 - (distance / 2) < 0, 0, x12 - (distance / 2)), # 1/2 distance for OUTER PADDING
         y3 = y12 + (distance/2), # 1/2 distance for OUTER PADDING
         x12 = (x12 + (distance / 4)), # 1/4 distance for INNER PADDING
...
index.distinct.tss.w.up.loop.each.end # any: 91559, within: 91558
...
df.overlapping.TSS.w.UPSTREAM.result.each.end %>% dim() # any: 91559     6
...
index.distinct.tss.w.down.loop.each.end # any: 106271, within: 106269
...
df.overlapping.TSS.w.DOWNSTREAM.result.each.end %>% dim() # 106271 (dedup) // 111334 (dups)
...
df.overlapping.TSS.w.BOTH.result %>% dim() # any: 197830      7 (dedup) // 207165 (dups)
...
tss.count.per.loop.histogram <- ggplot(tss_count_histogram_data, aes(x = tss_count_total)) +
  ...
  theme(plot.title = element_text(hjust = 0.5) +
        ...
        # metadata
        mcols(df.promoter.rn7.GR) <- df.promoter.rn7[, c("promoter.id", "center", "length")]
        ...
        ctcf_density_format_s <-chrctcf_density_format_s <- ctcf_density_s %>% dplyr::rename(Chr = chr, Start = start, End = end, Value = value) # %>% mutate(Value = Value/(End - Start))
        ...
        # %>% 
        #   mutate(loop.id = str_remove(loop.id, "_[^_]+$"))
        final.loops.from.ctcf.step # 18,327 + 10 | sub.4 any: 188,01, within: 18,737
        ...
        # 4-2. TSS
        df.overlapping.TSS.w.BOTH.result %>% head()
        df.tss.counts.each.end # 44564 (dedup) // 44,553 + 10
        tss.stats.by.resolution.each.end
        ...
        # %>% 
        #   mutate(loop.id = str_remove(loop.id, "_[^_]+$"))
        final.loops.from.tss.step # any: 17,046 (dedups) // 17,036 + 10
        ...
        # %>% 
        #   mutate(loop.id = str_remove(loop.id, "_[^_]+$"))
        final.loops.from.promoter.step # any: 13018 (dedups) // 13,003 + 10
        ...
        ggsave(filename = "./figures/submission/ctcf_vs_promoter_tss_venn_diagrams.pdf", plot = venn_plot_submission, width = 11, height = 8.5, units = "in")
        ...
        final.loops.from.ctcf.step # sub.4 any:18,801 | within: 18,737
        final.loops.from.promoter.step # sub.4 dedups any: 13,018
        final.loops.from.tss.step # sub.4 dedups any: 17,046
        ...
        overlapping_loops$ctcf_promoter_only # 730 :PASS// 659          | sub.4 679 dedup  // sub.4 any: 681| within 679
        overlapping_loops$ctcf_tss_only # 3266 :PASS// 3014             | sub.4 3092 dedups // sub.4 any: 3104 | 3096
        overlapping_loops$ctcf_promoter_tss_overlap # 8537 :PASS// 6964 | sub.4 8120 dedups // sub.4 any: 8137 | 8116
        ...
        df.final.loop # 12493/31773(0.3931955) /58992||| 11637/31773(0.3662544)/58992 ||| 11391/31773/58992 ||| any: 11922
        save(df.final.loop, file="./figures/submission/df_final_loop_sub.4.any.rda")
        write.csv(df.final.loop, file = "./figures/submission/df_final_loop_sub.4.dedup.any.csv", row.names = FALSE)
        ...
        df.final.loop.joined <- df.final.loop %>%
          left_join(df.loop.deep.sample.all, by = "loop.id")
        
        # any: 11922 (distinct) | 23978 (all samples)
        df.final.loop.joined %>% dim()
        df.final.loop.joined %>% distinct(loop.id)
        df.final.loop.joined %>% count(resolution)
        
        ############
        # filtering unreliable loops
        ############
        df.final.loop # 11922
        df.filtered.out.loop.bed # 9754
        df.final.loop.excluded <- anti_join(df.final.loop, df.filtered.out.loop.bed, by = "loop.id")
        df.final.loop.excluded %>% dim() # 9487
        ...
        # Output PDF file
        pdf(file = "./figures/submission/circos_loops_by_resolution.pdf", height = 11*0.8, width = 8.5*0.8)
        ...
        imgs <- list.files(path = "./figures/submission/resized_png/circos", pattern = "\\.png$", full.names = TRUE)
        img_list <- lapply(imgs, function(img) {
          image_read(img) %>% image_resize("1200x1200!")
        })
        combined_img <- image_append(image_join(img_list), stack = TRUE)
        image_write(combined_img, "./figures/submission/resized_png/circos/combined_circos.png", quality = 100)
